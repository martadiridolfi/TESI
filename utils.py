...
def LAhealthy_create_dataset(base_path,first_sample,last_sample):
    ...
    x = mat_1['data_train']['x']
    x = x[0][0][0][np.arange(0,333667,4)]
    y = mat_1['data_train']['y']
    y = y[0][0][0][np.arange(0,333667,4)]
    z = mat_1['data_train']['z']
    z = z[0][0][0][np.arange(0,333667,4)]
    
    #time_vec = np.arange(0,0.44,5e-5).T
    #print("Dimensione di time_vec: ", time_vec.shape)
    #time_vec = time_vec[np.arange(0,8800,20)]
    #print("Dimensione di time_vec: ", time_vec.shape)
    time_vec = 0.44*np.arange(0,1.0001,1.0/440).T
    #print("Dimensione di time_vec: ", time_vec.shape)
    ...
    for k in range(N_samples):
        ...
        mat = scipy.io.loadmat(base_path + '/sample_'+str(first_sample + k+1)+'.mat')
        vh = mat['data_train']['vh']
        output = vh[0][0][np.arange(0,333667,4)]
        ...
        for j in range(N_points):
            for i in range(N_times):
                dataset_matrix[k,i,j,0] = output[j,i]
                #timing = [0.0]
                #impulse_vec[k,i,0] = 0 #(time_vec[i]>=timing[0])*(time_vec[i]<=(timing[0]+0.003))

            points_matrix[j,0] = x[j]
            points_matrix[j,1] = y[j]
            points_matrix[j,2] = z[j]

        #diameter_vec[k,0] = param[0][0][3]
    
    dataset = {
        'points' : points_matrix , # [num_points x num_coordinates]
        'times' : time_vec , # [num_times]
        #'inp_parameters' : diameter_vec, # [num_samples x num_par]
        #'inp_signals' : impulse_vec, # [num_samples x num_times x num_signals]
        'out_fields' : dataset_matrix, # [num_samples x num_times x num_points x num_fields]
    }
